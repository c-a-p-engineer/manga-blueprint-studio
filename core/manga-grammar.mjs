...
const weight = (v='', fallback=.5) => {
  const s=String(v).trim().toLowerCase(); const n=Number(s);
  if(Number.isFinite(n)) return clamp01(n);
  if(/climax|maximum|最高|決め|最大/.test(s)) return 1;
  if(/high|strong|重要|強/.test(s)) return .8;
  if(/medium|normal|中|普通/.test(s)) return .5;
...
function parseImportance(value=''){
  ...
  for(const part of split(value)){
    const m=part.match(/^(narrative|story|物語|visual|絵|視覚|transition|遷移|つなぎ)\s*[=：:]\s*(.+)$/i);
    if(!m){ const w=weight(part,.5); out.narrative=w; out.visual=w; continue; }
    if(/narrative|story|物語/i.test(m[1])) out.narrative=weight(m[2]);
    else if(/visual|絵|視覚/i.test(m[1])) out.visual=weight(m[2]);
    else out.transition=weight(m[2]);
  }
  return out;
}
...
function parseSemanticPanels(text){
  ...
  for(const raw of text.split(/\r?\n/)){
    ...
    if(pm){ panel={shape:'rectangle',size:'auto',inset:'',pose:{},gaze:{},depth:{},support:{},motion:{},contacts:[],importance:{narrative:.5,visual:.5,transition:.5},timing:{hold:.5},attention:{primary:'',secondary:''},flow:{entry:'',exit:''}}; page.panels.push(panel); continue; }
    if(!panel) continue;
    ...
    else if((v=field('importance|重要度'))) panel.importance=parseImportance(v);
    else if((v=field('hold|間|滞留'))) panel.timing.hold=weight(v);
    else if((v=field('primary attention|primary|注目|主注目'))) panel.attention.primary=v;
    else if((v=field('secondary attention|secondary|副注目'))) panel.attention.secondary=v;
    else if((v=field('entry|視線入口'))) panel.flow.entry=v;
    else if((v=field('exit|視線出口'))) panel.flow.exit=v;
    ...
    else if((v=field('強調'))) { const w=weight(v); panel.importance.visual=Math.max(panel.importance.visual,w); panel.importance.narrative=Math.max(panel.importance.narrative,w*.85); }
  }
...
function quad(rect,preset){
  ...
  const pts={'diagonal-left':[{x:x+d,y},{x:x+w,y},{x:x+w-d,y:y+h},{x,y:y+h}],'diagonal-right':[{x,y},{x:x+w-d,y},{x:x+w,y:y+h},{x:x+d,y:y+h}],'trapezoid-left':[{x:x+d,y},{x:x+w,y},{x:x+w,y:y+h},{x,y:y+h}],'trapezoid-right':[{x,y},{x:x+w-d,y},{x:x+w,y:y+h},{x,y:y+h}]};
  return {kind:'quad',preset,points:pts[preset]||[{x,y},{x:x+w,y},{x:x+w,y:y+h},{x,y:y+h}]};
}
function normalizeShape(v=''){ if(/diagonal[- ]?left|斜め左/i.test(v)) return 'diagonal-left'; if(/diagonal[- ]?right|斜め右|斜め/i.test(v)) return 'diagonal-right'; if(/trapezoid[- ]?left|台形左/i.test(v)) return 'trapezoid-left'; if(/trapezoid[- ]?right|台形右|台形/i.test(v)) return 'trapezoid-right'; return 'rectangle'; }
