...
text = read(path)
pattern = r"const STARTER_CHARACTER_PRESETS_36=Object\.freeze\(\{.*?\n\}\);\nconst STARTER_CHARACTER_IDS_36"
block = """const STARTER_CHARACTER_PRESETS_36=Object.freeze({
...
const STARTER_CHARACTER_IDS_36"""
updated, count = re.subn(pattern, block, text, count=1, flags=re.S)
if count != 1:
    raise SystemExit(f'failed starter preset replacement: {count}')
old_appearance = "appearance:{summary:localized.summary,hair:'',eyes:'',outfit:localized.outfit,features:localized.features||''}"
new_appearance = "appearance:{summary:localized.summary,hair:localized.hair||'',eyes:localized.eyes||'',outfit:localized.outfit,features:localized.features||''}"
if old_appearance not in updated:
    raise SystemExit('starter appearance mapping not found')
updated = updated.replace(old_appearance, new_appearance, 1)
write(path, updated)

replace_once(
    'web/runtime/identity/character-guidance.js',
    "const lines=['','CHARACTER IDENTITY GUIDANCE:'];for(const ch of used){",
    "const lines=['','CHARACTER IDENTITY GUIDANCE:','- Planning stick figures are pose/placement guides only and do not indicate clothing state. Use each character\\'s explicit outfit guidance. If an outfit is unspecified, use ordinary scene-appropriate clothing rather than interpreting the stick figure as an unclothed body.'];for(const ch of used){",
)

replace_once(
    'web/runtime/handoff/render-brief.js',
...
replace_once(
    'web/runtime/handoff/render-brief.js',
    "doNotInherit:['stick-figure-appearance','authoring-labels'",
    "doNotInherit:['stick-figure-appearance','stick-figure-clothing-state','authoring-labels'",
)
replace_once(
    'web/runtime/handoff/render-brief.js',
...
replace_once(
    'web/guide.html',
    '<div class="callout">キャンバスの棒人間は<strong>見た目ではなくポーズ・位置・大きさの設計図</strong>です。</div>',
    '<div class="callout">キャンバスの棒人間は<strong>見た目や服装ではなくポーズ・位置・大きさの設計図</strong>です。棒人間を裸として解釈せず、キャラクターに設定した服装を完成絵へ使います。</div>',
)
replace_once(
    'web/guide.html',
    '<div class="callout"><strong>新規作品には6人のスターターがあります。</strong> 高校生男子 / 高校生女子 / 成人男性 / 成人女性 / モブ男性 / モブ女性。すべて編集可能な汎用のたたき台です。既存作品には自動追加せず、キャラタブの「かんたん追加」から必要なものを足せます。</div>',
    '<div class="callout"><strong>新規作品には6人のスターターがあります。</strong> 高校生男子 / 高校生女子 / 成人男性 / 成人女性 / モブ男性 / モブ女性。髪・目に加えて、学校制服や長袖トップス＋ロングパンツ等の服装まで初期指定した編集可能な汎用ベースです。既存作品には自動追加せず、キャラタブの「かんたん追加」から必要なものを足せます。</div>',
)

replace_once('package.json', '"version": "0.17.0"', '"version": "0.17.1"')
path = 'package-lock.json'
text = read(path)
needle = '"version": "0.17.0"'
if text.count(needle) < 2:
    raise SystemExit('package-lock expected two root version entries')
write(path, text.replace(needle, '"version": "0.17.1"', 2))
replace_once('web/build-info.json', '"appVersion": "0.17.0"', '"appVersion": "0.17.1"')
replace_once('web/src/main.ts', "appVersion:'0.17.0'", "appVersion:'0.17.1'")
replace_once('web/runtime/handoff/producer-provenance.js', "appVersion:'0.17.0'", "appVersion:'0.17.1'")
replace_once('README.md', 'Current prototype: 0.17.0', 'Current prototype: 0.17.1')
replace_once('docs/ROADMAP.md', 'Shipped through Prototype 0.17.0', 'Shipped through Prototype 0.17.1')

Path('docs/PROTOTYPE-0.17.1.md').write_text('''# Prototype 0.17.1 — explicit starter clothing and stick-figure handoff
