...
const editorShellText17={
  ja:{
    workLibrary:'作品管理',structure:'作品エクスプローラー',pageSettings:'ページ設定',pageOps:'ページ操作',newWork:'＋ 新しい作品',manageWorks:'作品管理',
  ...
  en:{
    workLibrary:'Manage works',structure:'Work explorer',pageSettings:'Page settings',pageOps:'Page actions',newWork:'+ New work',manageWorks:'Manage works',
...
function ensureEditorShell17(){
  ...
  $('explorerWorks17').addEventListener('click',async event=>{const button=event.target.closest('[data-explorer-work17]');if(!button)return;const id=button.dataset.explorerWork17;if(id&&id!==project?.meta?.workId)await openStoredWork16(id);renderEditorShell17();await renderExplorerWorks17();});
...
.editor-page-line17{display:grid;grid-template-columns:40px minmax(150px,auto) 40px 40px;gap:6px;align-items:center;margin-top:7px}.editor-page-line17 button{min-height:38px;padding:6px 10px}.compact17{font-size:20px;line-height:1}.editor-current-page17{font-weight:850;text-align:left}
...
@media(max-width:760px){
  ...
  .editor-context-shell17{position:static}.editor-context-main17{padding:8px 10px}.editor-work-title17{font-size:16px;max-width:88vw}.editor-page-line17{grid-template-columns:38px minmax(125px,1fr) 38px 38px}.work-explorer-grid17{grid-template-columns:1fr}.explorer-works17{max-height:26vh}.editor-page-line17 button{min-height:38px;padding:5px 8px}.editor-page-strip17{padding-top:6px}.page-strip-title17{display:none}.structure-dialog17{width:100vw;max-width:none;height:100dvh;max-height:none;border-radius:0;margin:0}.structure-shell17{max-height:100dvh;padding:14px}.structure-head17{top:-14px;padding-top:14px}
}
@media(max-width:430px){.editor-context-actions17 #workLibraryBtn16{font-size:11px}.editor-page-line17{grid-template-columns:36px minmax(110px,1fr) 36px 36px;gap:4px}.editor-page-line17 button{padding:4px 6px}}
`;
...
function renderEditorShell17(){
  ...
  $('currentWorkButton17').textContent=title;$('currentWorkButton17').title=esText17('structure');
  ...
  $('addPage17').title=esText17('addPage');$('newWorkExplorer17').textContent=esText17('newWork');$('manageWorksExplorer17').textContent=esText17('manageWorks');$('workExplorerWorksHeading17').textContent=language==='ja'?'作品':'Works';
  ...
  $('structureTree17').innerHTML=explorerTreeHtml17();$('structureTitle17').textContent=esText17('structure');$('structureTitle17').title=title;
  document.querySelector('.structure-kicker17').textContent=title;$('pageOpsSummary17').textContent=esText17('pageOps');void renderExplorerWorks17();
  localizeEditorResidue17();
}

async function openStructure17(){renderEditorShell17();await renderExplorerWorks17();const dialog=$('structureDialog17');if(dialog&&!dialog.open)dialog.showModal();}
